...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    gear = ((double)msg.channels[4] - 1000.0) / 1000.0;  // enable_cmd_mode
    mode = ((double)msg.channels[5] - 1000.0) / 1000.0;  // flgiht mode
    reboot_cmd = ((double)msg.channels[6] - 1000.0) / 1000.0; // do not use reboot cmd
    // emergency kill for channel_8 which is channels[7]

    // Add these 4 print lines (channels 4-7 + corresponding variables)
    // std::cout << "Ch 4: " << msg.channels[4] << ", Gear: " << std::fixed << std::setprecision(3) << gear;
    // std::cout << " Ch 5: " << msg.channels[5] << ", Mode: " << std::fixed << std::setprecision(3) << mode;
    // std::cout << " Ch 6: " << msg.channels[6] << ", Reboot: " << std::fixed << std::setprecision(3) << reboot_cmd << std::endl;
...
void Odom_Data_t::feed(nav_msgs::OdometryConstPtr pMsg)
{
    ...
    // std::cout << "we have received /odom" << std::endl;
    msg = *pMsg;
    rcv_stamp = now;
    recv_new_msg = true;
