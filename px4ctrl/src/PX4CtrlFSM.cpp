...
void PX4CtrlFSM::process()
{
	...
	if (state == AUTO_HOVER || state == CMD_CTRL)
	{
		...
		// controller.estimateThrustModel(imu_data.a,param); // ywy do not update thr2acc_ due to noisy IMU.acc
	...
	{
		// the program is always publish mavros_ctrl_cmd to keep staying in the offboard mode.
		// pub_count++;
		// if(pub_count%100==0){
		// 	std::cout << "we are publishg ctrl cmd, des is: "<< des.p(0) << des.p(1) << des.p(2) << std::endl;
		// }
		// publish_attitude_ctrl(u, now_time);  // ywy use position_ctrl
		publish_position_ctrl(des, now_time);
	}
	// ywy change this, direcly publish position ctrl cmd
...
{
	mavros_msgs::AttitudeTarget msg;
	// std::cout << "we are using GaoFei attitude_ctrl" << std::endl;
	msg.header.stamp = stamp;
	msg.header.frame_id = std::string("FCU");
...
// The new function to publish position control commands using Desired_State_t
void PX4CtrlFSM::publish_position_ctrl(const Desired_State_t& des, const ros::Time& stamp)
{
    mavros_msgs::PositionTarget msg;
	// std::cout << "we are using px4 position_ctrl" << std::endl;
    // 1. Header configuration
    msg.header.stamp = stamp;
    msg.header.frame_id = std::string("FCU"); // Use "local_origin" (NED) or "map" (ENU) based on your system
                                           // PX4 default is NED for local coordinates—ensure consistency with des.p/v/a

    // 2. Critical: Set coordinate frame and type mask
    // - FRAME_LOCAL_NED: Use NED (North-East-Down) coordinates (PX4 standard)
    // - TYPE_MASK: Specify which fields are VALID (unset bits = valid, set bits = ignored)
    msg.coordinate_frame = mavros_msgs::PositionTarget::FRAME_LOCAL_NED;
    msg.type_mask = 
        mavros_msgs::PositionTarget::IGNORE_AFX |  // Ignore if not using desired x-acceleration
        mavros_msgs::PositionTarget::IGNORE_AFY |  // Ignore if not using desired y-acceleration
        mavros_msgs::PositionTarget::IGNORE_AFZ |  // Ignore if not using desired z-acceleration
        // mavros_msgs::PositionTarget::IGNORE_JERK_X |  // PX4 typically ignores jerk
        // mavros_msgs::PositionTarget::IGNORE_JERK_Y |
        // mavros_msgs::PositionTarget::IGNORE_JERK_Z |
        mavros_msgs::PositionTarget::IGNORE_YAW_RATE;  // Unset if you want to use des.yaw_rate

    // 3. Populate desired states from Desired_State_t
    // Position (x, y, z) [m]
    msg.position.x = des.p.x();
    msg.position.y = des.p.y();
    msg.position.z = des.p.z();

    // Velocity (x, y, z) [m/s] (optional but recommended for smooth control)
    msg.velocity.x = des.v.x();
    msg.velocity.y = des.v.y();
    msg.velocity.z = des.v.z();

    // Acceleration (x, y, z) [m/s²] (optional—PX4 uses this for feedforward)
    msg.acceleration_or_force.x = des.a.x();
    msg.acceleration_or_force.y = des.a.y();
    msg.acceleration_or_force.z = des.a.z();

    // Yaw and yaw rate [rad], [rad/s]
    msg.yaw = des.yaw;                  // Desired heading (0 = North for NED frame)
    msg.yaw_rate = des.yaw_rate;        // Desired yaw rate (only valid if IGNORE_YAW_RATE is unset)

    // 4. Publish the message
    pos_setpoint_pub.publish(msg);
}
